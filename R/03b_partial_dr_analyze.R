...
## (a) 45-degree tracking (pooled per-rep + cell means)
r_rep  <- cor(pool[,"bias"], pool[,"b_UA"])
fit_cell <- lm(mean_bias ~ mean_bUA, data=G)
cat(sprintf("\n(a) FORMULA TRACKS: per-rep cor(bias, b_UA) = %.3f;  cell-mean slope = %.3f, intercept = %.3f (ideal 1, 0)\n",
...
cat(sprintf("(c) oracle plug true U: mean|bias_oracle| over cells = %.3f (should ~0)\n",
...
## panel 1: 45-degree scatter (pooled reps, colored light) + cell means (bold)
...
legend("topleft", bty="n", cex=0.8, legend=c("per-rep","cell mean","y = x"),
